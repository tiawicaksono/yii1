...
class AssignmentController extends RController {
     ...
     */
    public function actionView($name = '', $company = '') {
        $criteria = new CDbCriteria();

        if (strlen($name) > 0 || count($company) > 0) {
            $criteria->addSearchCondition('upper("t".name)', strtoupper($name), true, 'AND');
        }
...
//    public function actionView($name = '', $company = '') {
//        $criteria = new CDbCriteria();
//        $criteria->with = array('employee.company_rel');
//        $criteria->together = TRUE;
//        $criteria->addInCondition('"t".STATUS_ID', array(1));
//
//        if (strlen($name) > 0 || count($company) > 0) {
//            $criteria->addSearchCondition('"employee"."company_rel".MAIN_COMPANY_ID', $company, true, 'AND');
//            $criteria->addSearchCondition('upper("t".LOGIN_NAME)', strtoupper($name), true, 'AND');
//        }
//        // Create a data provider for listing the users
//        $dataProvider = new RAssignmentDataProvider(array(
//            'criteria' => $criteria,
